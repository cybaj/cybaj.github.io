...
명제는 네 개다. 질량은 보존된다, 선운동량은 평형을 이룬다, 각운동량도 평형을
이룬다, 에너지는 보존된다. 어느 것도 특정한 재료에 관한 이야기가 아니어서
강철이든 고무든 물이든 똑같이 따른다. 이들을 갈라놓는 요소는
[구성 방정식]({{< ref "constitutive.md" >}})까지 등장하지 않는다.
(개인 노트: preparation 1 : 기본 보존 법칙)

각 법칙은 유한한 크기의 물질 덩어리에 대한 진술로 시작해서 모든 점에서
성립하는 편미분 방정식으로 끝난다. 그 변환에 쓰이는 도구가 매번 같으므로
그것부터 준비한다.
...
보존 법칙이 말하는 것은 *물질* 영역이 담고 있는 양의 변화율이다. 물질 영역은
언제나 같은 입자들로 이루어져 있으므로 그 경계가 함께 움직인다. 움직이는 영역
위의 적분을 미분하는 일은 만만치 않다. 피적분 함수도 적분 영역도 시간에 따라
변하기 때문이다.

요령은 영역을 고정시키는 것이다. 임의의 공간 장
...
{{< katex >}}\Omega_0{{< /katex >}} 가 시간에 대해 고정되어 있기 때문이다.
곱의 미분법으로 전개하고 [운동학]({{< ref "kinematics.md" >}})에서 얻은
{{< katex >}}\dot{J} = J \nabla \cdot \mathbf{v}{{< /katex >}} 를 넣는다.
...
이것이 **레이놀즈 수송 정리**(Reynolds transport theorem)다. 첫째 항은 양
자체가 변해서 생기는 몫이고, 둘째 항은 영역이 커지거나 작아져서 생기는 몫이다.

### 국소화

아래의 유도는 모두
{{< katex >}}\int_{\Omega_t} (\cdots) \, \mathrm{d}v = 0{{< /katex >}} 형태에
도달한 다음, 피적분 함수가 점마다 0 이라고 결론짓는다. 이 단계의 근거는 한
번은 짚고 넘어가야 한다.

적분이 0 인 것은 어느 한 영역에 대해서가 아니라 *모든* 물질 영역에 대해서다.
보존 법칙이 애초에 특정 덩어리를 두고 한 말이 아니기 때문이다. 어떤 점
{{< katex >}}\mathbf{x}_0{{< /katex >}} 에서 피적분 함수가 양수라고 해 보자.
연속이므로 그 점 주위의 작은 공 안에서도 계속 양수일 것이고, 그 공만 적분하면
0 보다 큰 값이 나와 가정과 어긋난다. 음수인 경우도 같은 방식으로 배제된다.

따라서 피적분 함수는 어디서나 0 이다. 단, 연속이라는 조건이 붙는다. 이 조건은
결국 연속체 가정이 다시 등장한 것이며, 충격파처럼 불연속이 있는 곳에서는
성립하지 않는다. 그런 곳에서는 적분 형태만 살아남고 편미분 방정식은 살아남지
못한다.
...
**연속 방정식**(continuity equation)이다. 물질 미분을 풀어 쓰면 공간 형태
...
{{< katex >}}\rho_0{{< /katex >}} 는 입자에 붙어 있는 값이라 시간에 따라 변할
수 없으므로
...
이고 {{< katex >}}J > 0{{< /katex >}} 이므로 괄호 안이 0 이다. 반대편에서
출발했지만 같은 식에 도착했다.

### 숫자로 확인하기

밀도가 {{< katex >}}1000 \ \mathrm{kg/m^3}{{< /katex >}} 인 젤리 덩어리를 눌러
부피를 절반으로 만든다.
...
부피는 절반이 되고 밀도는 두 배가 되며 질량은 그대로다.
(개인 노트: preparation 1 : 기본 보존 법칙) 연속 방정식은 이 사실을 변화율의
언어로 말하고, {{< katex >}}\rho_0 = \rho J{{< /katex >}} 는 한 줄로 말한다.

## 2. 코시 응력

운동량으로 넘어가기 전에, 물체 내부의 면에 걸리는 힘을 표현할 방법이 필요하다.

단위 법선이 {{< katex >}}\mathbf{n}{{< /katex >}} 인 면을 따라 물체를 자른다.
한쪽 물질이 다른 쪽을 단위 면적당 얼마의 힘으로 당기는지를
**트랙션**(traction) {{< katex >}}\mathbf{t}{{< /katex >}} 라 한다. 이 값은
자른 방향에 따라 달라진다. 같은 점이라도 가로로 자른 면과 세로로 자른 면의
트랙션은 다르다.

코시가 밝힌 것은 그 의존성이 *선형*이라는 사실이다. 세 면은 좌표평면 위에
놓이고 나머지 한 면의 법선이 {{< katex >}}\mathbf{n}{{< /katex >}} 인 작은
사면체를 생각해 보자. 힘의 평형을 쓰면 표면에 관한 항은
{{< katex >}}L^2{{< /katex >}} 에, 체적력과 관성에 관한 항은
{{< katex >}}L^3{{< /katex >}} 에 비례한다.
...
가 남는다. {{< katex >}}\sigma_{ji}{{< /katex >}} 는 법선이
{{< katex >}}\mathbf{e}_j{{< /katex >}} 인 면에 걸리는 트랙션의
{{< katex >}}i{{< /katex >}} 성분이다. 이것을 **코시 응력 텐서**(Cauchy stress
tensor)라 하며, 단위 *현재* 면적당 힘, 즉 실험에서 실제로 재는 응력이다.

첨자 순서를 이렇게 적은 데는 이유가 있고, 잠시뿐이다. 보통은
{{< katex >}}\mathbf{t} = \boldsymbol{\sigma}\mathbf{n}{{< /katex >}} 이라
쓰지만 그 표기는 대칭성을 전제한다. 그런데 대칭성은 아직 증명되지 않았고,
바로 다음의 각운동량에서 유도할 참이다. 여기서 미리 가정해 버리면 그 유도가
순환 논증이 된다. 그래서 4절까지는 첨자를 그대로 드러낸다.

## 3. 선운동량

물질 영역에 뉴턴 제2법칙을 적용한 것이다. 운동량의 변화율은 가해진 힘과 같고,
힘은 경계에 걸리는 트랙션과 중력 같은 단위 질량당 체적력
...
묶인 괄호가 연속 방정식이라 통째로 사라진다. 밀도가 얼마든지 변할 수 있는데도
운동량 평형이 결국 {{< katex >}}m\mathbf{a}{{< /katex >}} 꼴로 정리되는 것은
이 상쇄 덕분이다.
...
되고, 모든 응력 해석이 결국 푸는 것이 이 식이다. 유한요소법이 시험 함수와
곱해 부분적분하는 대상도, 물리 기반 신경망이 손실 함수에서 벌점을 매기는
대상도 같은 식이다.

### 기준 상태에서 다시 쓰기

방금 얻은 식에는 현실적인 문제가 있다. 변형된 물체
{{< katex >}}\Omega_t{{< /katex >}} 위에서 세워졌는데, 그 영역은 문제를 풀기
전에는 알 수 없다. 대변형에서는 이 순환이 치명적이다. 해결책은 이미 알고 있는
영역인 {{< katex >}}\Omega_0{{< /katex >}} 위에서 법칙을 다시 쓰는 것이다.

면적분을 옮기려면 현재 면적 요소와 기준 면적 요소를 이어 주는 관계가 필요하다.
**난슨 공식**(Nanson's formula)이 그 역할을 한다.

{{< katex display=true >}}
\mathbf{n} \, \mathrm{d}a = J \mathbf{F}^{-T} \mathbf{N} \, \mathrm{d}A
{{< /katex >}}

유도는 {{< katex >}}\mathrm{d}v = J \, \mathrm{d}V{{< /katex >}} 에서 출발한다.
기준 상태의 면적 요소 {{< katex >}}\mathbf{N} \, \mathrm{d}A{{< /katex >}} 와
임의의 벡터 {{< katex >}}\mathrm{d}\mathbf{X}{{< /katex >}} 를 잡으면 이 둘이
부피 {{< katex >}}\mathrm{d}V = \mathbf{N} \cdot \mathrm{d}\mathbf{X} \, \mathrm{d}A{{< /katex >}}
인 기둥을 이룬다. 변형 후 같은 물질이 차지하는 부피는
...
이제 *같은 힘*을 기준 상태의 면적으로 나누어 표현하기로 한다.
**제1 피올라-키르히호프 응력**(first Piola–Kirchhoff stress)
{{< katex >}}\mathbf{P}{{< /katex >}} 를 다음과 같이 정의한다.
...
이 정의 덕분에 운동량 평형은 모양을 그대로 유지한 채 기준 상태로 옮겨 간다.
...
기준 상태에, 한 발은 현재 상태에 걸치고 있다. 그래서 **대칭이 아니다**.
실제로 불편한 성질이고, [응력]({{< ref "stress.md" >}})에서 세 번째 응력
텐서를 도입해 해결한다.
...
압축이 더해진다. 평형 방정식은 추상적인 이야기가 아니다. 높은 기둥의 아래쪽이
위쪽보다 큰 응력을 받는 이유이고, 그 증가율까지 정확히 정해 준다.
...
여기서 나오는 것은 또 하나의 미분 방정식이 아니라 대수적인 제약이다. 외적
때문에 직접 표기로는 다루기 번거로우므로 인덱스 표기를 쓴다.
...
형식적인 유도만 보면 정작 단순한 역학적 사실이 가려진다. 한 변이
{{< katex >}}L{{< /katex >}} 인 정육면체에서
{{< katex >}}\sigma_{12} \neq \sigma_{21}{{< /katex >}} 이라고 해 보자. 마주
보는 면의 전단 트랙션이 서로 상쇄되지 않는 짝힘을 이루어 알짜 토크가 남는다.
...
{{< katex >}}L{{< /katex >}} 을 곱한 값이다. 한편 정육면체의 관성 모멘트는

{{< katex display=true >}}
I \sim \rho L^3 \cdot L^2 = O(L^5)
{{< /katex >}}

질량 {{< katex >}}\rho L^3{{< /katex >}} 에 길이의 제곱을 곱한 값이다. 따라서
각가속도는
...
아무리 작은 불균형이라도 미소 요소를 무한대의 각가속도로 돌려 버린다. 물질은
그렇게 행동하지 않으므로 불균형은 존재할 수 없다. 지수의 차이, 즉 3 과 5 의
차이가 논증의 전부이며, 이 제약이 근사가 아니라 정확한 이유도 거기에 있다.

## 5. 열역학 제1법칙

영역에 들어온 에너지는 그 영역이 가진 에너지의 변화와 같다. 기계적 일률은
트랙션과 체적력을 통해, 열은 경계를 지나는 플럭스
{{< katex >}}\mathbf{q}{{< /katex >}} 와 단위 질량당 공급량
{{< katex >}}r{{< /katex >}} 을 통해 들어온다. 저장된 에너지는 운동 에너지와
...
플럭스 앞의 음부호는 약속이다.
{{< katex >}}\mathbf{q}{{< /katex >}} 는 열이 *빠져나가는* 방향을 향한다.

트랙션에 의한 일률을 정리한다. 이제 정당해진
...
가 힘을 발휘한다. {{< katex >}}\boldsymbol{\sigma}{{< /katex >}} 는 대칭이고
...
가운데 단계에서 더미 첨자를 바꿔 붙였다. 결국

{{< katex display=true >}}
\boldsymbol{\sigma}:\mathbf{L} = \boldsymbol{\sigma}:\mathbf{D}
{{< /katex >}}

**회전에는 에너지가 들지 않는다.** 이것이 이 식의 물리적 내용이고, 방금
각운동량에서 얻은 대칭성이 곧바로 만들어 낸 결과다.

마지막으로 기계적 에너지 항등식을 뺀다. 선운동량 평형에
...
이 그것이다. 양변에서 운동 에너지 항과 응력 발산 항이 상쇄되고, 국소화하면
...
일률**(stress power)이라 한다. 단위 현재 부피당, 변형이 재료에 에너지를 넣는
비율이다. 이후 내용으로 이어지는 통로이기도 해서,
[응력]({{< ref "stress.md" >}})은 이 값에 세 가지 표현이 있음을 보이고
[구성 방정식]({{< ref "constitutive.md" >}})은 이 항 때문에 에너지 함수를
미분해 응력을 얻을 수 있음을 보인다.

## 미지수와 방정식을 세어 보면

지금까지 얻은 것은 질량에서 스칼라 방정식 하나, 선운동량에서 셋, 각운동량에서
대칭 제약, 에너지에서 스칼라 하나다.

열적 효과를 빼고 세어 보자. 미지수는
{{< katex >}}\rho{{< /katex >}} 하나,
{{< katex >}}\mathbf{v}{{< /katex >}} 셋,
{{< katex >}}\boldsymbol{\sigma}{{< /katex >}} 여섯(대칭성 적용 후)으로 모두
열 개다. 방정식은 질량 하나와 운동량 셋으로 네 개다.

**여섯 개가 모자란다.** 보존 법칙을 더 파고들어도 이 간격은 메워지지 않는다.
그 자리가 바로 재료를 아직 정하지 않은 자리이기 때문이다. 메우는 일은
