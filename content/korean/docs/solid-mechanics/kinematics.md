...
힘보다 변형이 먼저입니다. 여기서는 물체의 모양이 어떻게 바뀌는지만 다루고 왜
바뀌는지는 묻지 않습니다. 이하의 결과는 물체가 강철이든 고무든 젤리든 그대로
성립합니다.

## 두 개의 기준과 표기 약속

물질을 이루는 각 입자에, 아무 일도 일어나기 전에 있던 자리를 이름으로
붙입니다. 기준 상태(reference configuration)
{{< katex >}}\Omega_0{{< /katex >}} 에서의 위치
{{< katex >}}\mathbf{X}{{< /katex >}} 는 그 입자의 이름이지 계속 머무는 자리가
아닙니다. 시각 {{< katex >}}t{{< /katex >}} 에 같은 입자는
...
이후 모든 문서에 걸쳐 지키는 약속을 먼저 정해 두겠습니다.

> 대문자는 기준 상태, 소문자는 현재 상태를 가리킵니다.
...
기술 방식도 두 가지로 나뉩니다. **물질 기술**(Lagrangian description)은 양을
{{< katex >}}(\mathbf{X}, t){{< /katex >}} 의 함수로 주며, 입자 하나를 따라
가며 보는 방식입니다. **공간 기술**(Eulerian description)은
{{< katex >}}(\mathbf{x}, t){{< /katex >}} 의 함수로 주며, 공간의 한 지점에
서서 그곳을 지나가는 것을 기록하는 방식입니다.

고체는 대개 물질 기술을 씁니다. 기준 상태는 이미 알고 있고 현재 상태가 구하려는
답이기 때문입니다. 유체는 대개 공간 기술을 씁니다. 수도꼭지에서 나오는 물이 어느
입자인지는 궁금하지 않기 때문입니다.
...
{{< katex >}}f(\mathbf{x}, t){{< /katex >}} 를 공간 장이라 하겠습니다. *입자가
겪는* 변화율은 그 입자의 운동을 대입한 다음 미분해서 얻습니다.
...
를 썼습니다. 앞으로 점을 찍은 기호는 언제나 이것, 즉
{{< katex >}}\mathbf{X}{{< /katex >}} 를 고정하고 취하는 **물질 시간
미분**(material time derivative)을 뜻합니다.

둘째 항 {{< katex >}}\mathbf{v} \cdot \nabla f{{< /katex >}} 는 입자가 다른
자리로 옮겨 갔다는 사실만으로 겪는 변화입니다. 강물은 정상 상태일 수 있습니다.
...
끝납니다. 가까이 있는 두 입자를 {{< katex >}}\mathbf{X}{{< /katex >}} 와
{{< katex >}}\mathbf{X} + \mathrm{d}\mathbf{X}{{< /katex >}} 에 놓겠습니다.
변형 후의 간격은 운동을 1차까지 테일러 전개하면 나옵니다.
...
여기 나타난 행렬이 **변형 구배 텐서**(deformation gradient)입니다.
...
{{< katex >}}J{{< /katex >}} 로 섞여 있는 데는 이유가 있습니다.
{{< katex >}}\mathbf{F}{{< /katex >}} 는 **두 점 텐서**(two-point tensor)여서
기준 상태의 벡터를 받아 현재 상태의 벡터를 내놓습니다. 어느 한쪽에 속하지
않기 때문에 뒤에서 까다롭게 구는 것입니다.

국소적인 변형에 관한 정보는 모두
{{< katex >}}\mathbf{F}{{< /katex >}} 안에 들어 있습니다. 아래에 나오는 다른
척도는 전부 여기서 만들어집니다.
...
{{< katex >}}J{{< /katex >}} 는 **야코비안**(Jacobian), 즉 국소적인
부피비입니다. {{< katex >}}J = 1{{< /katex >}} 이면 부피가 유지됩니다.

{{< katex >}}J{{< /katex >}} 에 붙는 제약 두 가지는 대수가 아니라 물리에서
옵니다. 우선 0 이 될 수 없습니다. {{< katex >}}J = 0{{< /katex >}} 은 유한한
부피를 한 점으로 눌러 없애는 일이기 때문입니다. 음수도 될 수 없습니다.
행렬식이 음수라는 것은 방향이 뒤집힌다는 뜻이고, 그러려면 물체가 자기 자신을
뚫고 지나가야 하기 때문입니다. 따라서

{{< katex display=true >}}
J > 0 \quad \text{(언제나)}
{{< /katex >}}

이고, {{< katex >}}\mathbf{F}{{< /katex >}} 는 어디서나 역행렬을 갖습니다.
이 가역성은 앞으로 계속 쓰이는데, 기술적인 편의가 아니라 물리적인 사실로
기억해 두는 편이 좋습니다.

### 질량과 야코비안

질량은 생기지도 사라지지도 않으므로, 물질 덩어리의 질량은 어느 기준에서
계산해도 같아야 합니다.
...
미분이 하나도 없는, 가장 간결한 형태의 질량 보존입니다.
...
각각 우 신장 텐서와 **좌 신장 텐서**(right/left stretch tensor)라 합니다.

증명은 짧습니다. {{< katex >}}\mathbf{F}^T\mathbf{F}{{< /katex >}} 는 대칭이고,
{{< katex >}}\mathbf{a} \neq \mathbf{0}{{< /katex >}} 에 대해
{{< katex >}}\mathbf{a} \cdot \mathbf{F}^T\mathbf{F}\mathbf{a} = |\mathbf{F}\mathbf{a}|^2 > 0{{< /katex >}}
이므로(여기서도 가역성이 쓰입니다) 양의 정부호입니다. 대칭 양의 정부호
행렬은 대칭 양의 정부호인 제곱근을 유일하게 가지므로
...
가 되어 {{< katex >}}\mathbf{R}{{< /katex >}} 이 직교 행렬임이 확인됩니다.

물리적으로 말하면, 어떤 변형이든 순수한 신장 다음의 회전이거나 회전 다음의
또 다른 순수한 신장입니다. 아주 작은 영역에서 일어날 수 있는 일은 그것뿐입니다.

## 변형률 척도

분해가 알려 주는 바는 {{< katex >}}\mathbf{R}{{< /katex >}} 을 버리고
{{< katex >}}\mathbf{U}{{< /katex >}} 만 남기라는 것입니다. 그런데
{{< katex >}}\mathbf{U}{{< /katex >}} 를 뽑아내려면 행렬 제곱근을 계산해야
해서 번거롭습니다. 그래서 대신 그 제곱을 쓰는데, 이것이 **우 코시-그린
텐서**(right Cauchy–Green tensor)입니다.

{{< katex display=true >}}
\mathbf{C} = \mathbf{F}^T\mathbf{F} = \mathbf{U}\mathbf{R}^T\mathbf{R}\mathbf{U} = \mathbf{U}^2
{{< /katex >}}

회전이 상쇄되었고 제곱근은 쓰지 않았습니다.
{{< katex >}}\mathbf{C}{{< /katex >}} 는 첨자가 둘 다 대문자로, 온전히 기준
상태에 속합니다.

의미는 길이 제곱의 변화입니다. 기준 상태의 선분
...
유지한다는 뜻이고, 그것이 곧 "변형이 없다" 는 말입니다. 이 기준선을 빼면
**그린-라그랑주 변형률 텐서**(Green–Lagrange strain tensor)가 됩니다.
...
{{< katex >}}\mathbf{E} = \mathbf{0}{{< /katex >}} 이 됩니다. 앞의
{{< katex >}}\tfrac{1}{2}{{< /katex >}} 는 미소 변형에서 공학 변형률과 값이
맞아떨어지도록 붙인 것이고, 아래에서 확인하겠습니다.

현재 상태에도 같은 구성이 있습니다.
{{< katex >}}\mathbf{b} = \mathbf{F}\mathbf{F}^T = \mathbf{V}^2{{< /katex >}} 를
좌 코시-그린 텐서라 하며, [구성 방정식]({{< ref "constitutive.md" >}})에서
쓰입니다.
...
학부에서 배우는 탄성론은 이 식을 선형화한 것입니다. 그리고 방금 버린
2차항이야말로 선형 이론을 무리하게 밀어붙였을 때 회전을 변형률로 착각하게
만드는 항입니다.

## 변화율: 속도 구배, 변형률 속도, 스핀

이후 내용에는 변형뿐 아니라 변형의 속도가 필요합니다. 공간 속도장을 공간으로
미분해 **속도 구배 텐서**(velocity gradient)를 정의합니다.

{{< katex display=true >}}
\mathbf{L} = \frac{\partial \mathbf{v}}{\partial \mathbf{x}}, \qquad L_{ij} = \frac{\partial v_i}{\partial x_j}
{{< /katex >}}

{{< katex >}}\dot{\mathbf{F}}{{< /katex >}} 와의 관계는 편미분의 순서를
바꾸면 바로 나옵니다.
...
{{< katex >}}\mathbf{W}{{< /katex >}} 를 **스핀**(spin)이라 합니다. 물체가
통째로 회전할 때 {{< katex >}}\mathbf{D}{{< /katex >}} 는 사라지고
{{< katex >}}\mathbf{W}{{< /katex >}} 는 남습니다. 변형을 담당하는 쪽이
{{< katex >}}\mathbf{D}{{< /katex >}}, 회전을 담당하는 쪽이
{{< katex >}}\mathbf{W}{{< /katex >}} 인 것입니다.
[응력]({{< ref "stress.md" >}})에서는 이 분해가 회전에는 에너지가 들지
않는다는 결론으로 이어집니다.

### 야코비안의 변화율

[보존 법칙]({{< ref "balance.md" >}})에서 쓸 항등식이 하나 더 있습니다.
행렬식의 미분에 관한 야코비 공식으로부터
...
세 번째 등호에서 대각합의 순환 성질을 썼습니다. 뜻은 분명합니다. 부피는
속도의 발산만큼의 비율로 늘어나며, 비압축성 운동이란
{{< katex >}}\nabla \cdot \mathbf{v} = 0{{< /katex >}} 인 운동입니다.

## 숫자로 확인하기

한 변의 길이가 1인 고무판을 가로로 두 배 늘이고 세로로 절반으로 누릅니다.
...
{{< katex >}}x_2{{< /katex >}} 방향으로 줄어들었음을 말해 줍니다. 다만
{{< katex >}}\mathbf{E}{{< /katex >}} 는 공학 변형률
{{< katex >}}\operatorname{diag}(1.0, -0.5){{< /katex >}} 와 다릅니다. 100%
신장에서는 앞서 버렸던 2차항이 1차항과 같은 크기가 되어 두 척도가 갈라지기
때문입니다. 이 구별이 말장난이 아니게 되는 영역이 바로 여기입니다.

### 회전을 넣어 보면

이제 이 모든 구성의 출발점이었던 경우입니다. 변형 없이
{{< katex >}}90^\circ{{< /katex >}} 회전만 주겠습니다.
...
근사적으로 0 에 가까운 것이 아니라 정확히 0 입니다. 반면
{{< katex >}}\mathbf{F} - \mathbf{I}{{< /katex >}} 의 성분은 크기가 1 이나
됩니다. 어설픈 변형률 척도였다면 손도 대지 않은 물체에 100% 변형률이 있다고
보고했을 것입니다. 그것을 없애 준 것이
{{< katex >}}\mathbf{F}^T\mathbf{F}{{< /katex >}} 의 전치입니다.

모양 변화를 기술했고 회전도 따로 떼어 놓았으니, 이제
[보존 법칙]({{< ref "balance.md" >}})에서 힘을 다룰 수 있습니다.
