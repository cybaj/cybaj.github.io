...
나눈
{{< katex >}}\mathbf{P} = J\boldsymbol{\sigma}\mathbf{F}^{-T}{{< /katex >}} 다.
여기에 세 번째인 {{< katex >}}\mathbf{S}{{< /katex >}} 가 곧 추가된다.

하나의 물리적 대상에 텐서가 셋이면 자연스레 묻게 된다. 어느 것이 *진짜*
응력인가. 답은 질문 자체가 잘못 세워졌다는 것이고, 셋이 같은 물리를 기술한다는
말의 정확한 의미가 **일 켤레성**(work conjugacy)이다. 응력과 변형률 속도는
짝을 이룰 때만 의미를 가지며, 올바르게 짝지어진 모든 조합은 같은 값을 내놓는다.
재료로 들어가는 단위 시간당 에너지다.
(개인 노트: preparation 2 : 일 켤레성(Work Conjugacy)과 일률 유도)

## 하나로 부족한 이유

면을 통해 전달되는 힘은 물리적 사실이다. "응력" 은 그 힘을 면적으로 나눈
값인데, 물체가 크게 변형하는 순간 나눌 면적이 두 개가 된다.

고무줄을 당겨 보자. 늘어나면서 가늘어지므로 지금의 단면적은 처음보다 작다.
같은 장력을 더 작아진 단면적으로 나누면 처음 단면적으로 나눈 값보다 큰
숫자가 나온다. 둘 다 맞는 값이며, 서로 다른 질문에 답하고 있을 뿐이다.

- {{< katex >}}\boldsymbol{\sigma}{{< /katex >}}, **코시 응력**은 현재
  면적을 쓴다. 재료가 실제로 겪는 값이고 항복 조건을 쓸 때의 기준이다.
  진응력(true stress)이라고도 한다.
- {{< katex >}}\mathbf{P}{{< /katex >}}, **제1 피올라-키르히호프 응력**은 기준
  면적을 쓴다. 시험기가 측정한 하중을 처음 단면적으로 나눠 보고하는 값이며,
  더 중요하게는 이미 알고 있는 영역 위에서 평형식을 세울 수 있게 해 준다.
  공칭응력(engineering stress)이라고도 한다.

{{< katex >}}\mathbf{P}{{< /katex >}} 를 쓰는 실질적인 이유는 대변형 문제를
{{< katex >}}\Omega_t{{< /katex >}} 위에서 적분할 수 없다는 데 있다. 그
영역이 곧 미지수이기 때문이다. 결국 문제는
{{< katex >}}\Omega_0{{< /katex >}} 위에서 세워야 한다. 그런데
{{< katex >}}\mathbf{P}{{< /katex >}} 는 대칭이 아니어서 저장 공간도 더 들고
대칭 고윳값 문제의 이점도 누리지 못한다. 세 번째 텐서가 필요한 이유가 이것이다.
...
이다. {{< katex >}}\mathrm{d}v = J\,\mathrm{d}V{{< /katex >}} 로 단위 *기준*
부피 기준으로 환산하면 응력 일률은
...
가 된다. 이후의 모든 계산은 이 한 값을 값의 변화 없이 다른 모습으로 바꾸는
작업이다.
...
스칼라인 {{< katex >}}J{{< /katex >}} 는 대각합 안팎을 자유롭게 오갈 수 있고,
앞의 두 인수가 정확히 {{< katex >}}\mathbf{P}{{< /katex >}} 의 정의로 묶인다.
...
{{< katex >}}\dot{\mathbf{F}}{{< /katex >}} 와 일 켤레 관계다. 눈여겨볼 것은
{{< katex >}}\mathbf{P}{{< /katex >}} 가
...
그 다리를 떼어 내면 **제2 피올라-키르히호프 응력**(second Piola–Kirchhoff
stress)이 된다.
...
{{< katex >}}\mathbf{E}{{< /katex >}} 와 마찬가지로 온전히 기준 상태에 속한다.

대신 해석하기가 어려워진다.
{{< katex >}}\mathbf{S}{{< /katex >}} 는 무언가의 단위 면적당 힘이 아니다.
힘까지 기준 상태로 끌어온
{{< katex >}}\boldsymbol{\sigma}{{< /katex >}} 여서 어떤 계측기로도 잴 수
없다. 계산을 위해 만들어진 대상이며, 그 목적에는 대단히 유용하다.
...
이다. {{< katex >}}\mathbf{M}{{< /katex >}} 의 반대칭 부분과의 축약이
사라지기 때문인데,
...
식으로는 전혀 다른 셋이지만 값은 하나다. 계산에서 어느 것을 쓸지는 편의의
문제이지 물리의 문제가 아니다.
...
부피당 일률이 된다. [객관성]({{< ref "objectivity.md" >}})에서 주로 쓰는
변수다.
...
세 식이 같은 값을 준다는 주장이야말로 실제 행렬을 넣어 확인해 볼 만하다.
...
대칭으로 떨어졌다. [보존 법칙]({{< ref "balance.md" >}})에서
{{< katex >}}\boldsymbol{\sigma}{{< /katex >}} 가 대칭이어야 함을 증명했으니
계산이 맞다는 별도의 확인이 된 셈이다. 다음으로
...
**세 번 모두 3.0 이다.** 세 경로는 중간에 거친 값을 하나도 공유하지 않았다.
...
다르다. 그런데도 결과가 일치한다는 것이 곧 정리의 내용이고, 그것이 눈앞에
드러난 것이다.

무엇이 변하지 않는 값인지도 함께 보인다. 응력의 성분은 어느 기준을 택하느냐에
따라 달라지므로, 기준을 밝히기 전에는 아무 의미가 없다. 반면 일률은 달라지지
않는다. **여기서 물리적인 것은 에너지**이며,
[구성 방정식]({{< ref "constitutive.md" >}})이 에너지 함수 하나를 쓰고
미분하는 것만으로 재료를 정의할 수 있는 근거도 여기에 있다.

한 가지 다루지 않은 것이 남았다. 응력의 변화율이다. 일률 항등식에 등장한
{{< katex >}}\dot{\mathbf{F}}{{< /katex >}} 와
{{< katex >}}\dot{\mathbf{E}}{{< /katex >}} 는 *변형*의 변화율이라 문제될 것이
없지만, 많은 재료 법칙은
{{< katex >}}\dot{\boldsymbol{\sigma}}{{< /katex >}} 를 필요로 한다. 그리고 그
미분에는 문제가 있다. [객관성]({{< ref "objectivity.md" >}})에서 다룬다.
